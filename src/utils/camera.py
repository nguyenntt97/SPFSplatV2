...
def rotate_image(image, camera=None):
    # Use NumPy's faster transpose and flip operations instead of skimage.rotate
    ...
    if camera is not None:
        ...
        return rotated_image, camera
    else:
        return rotated_image

def rotate_image_fast(image, angle=90, camera=None):
    """
    Faster image rotation using NumPy operations.
    
    Args:
        image: Input image to rotate
        angle: Rotation angle in degrees (only supports multiples of 90)
        camera: Optional camera dictionary to update
    
    Returns:
        Rotated image and updated camera if provided
    """
    # Convert angle to number of 90-degree rotations
    k = (angle // 90) % 4
    
    # Use NumPy's rot90 which is much faster than skimage.rotate
    ...
    # Update camera parameters based on rotation
    if k % 2 == 1:  # 90 or 270 degrees
