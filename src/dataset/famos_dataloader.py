...
from utils.utils import get_filename, timeit


# Helper functions for directory and file path construction
def get_img_dir_from_image_dir(image_dir, subject, sequence, frame):
    return os.path.join(image_dir, subject, sequence, frame)

def get_img_dir_from_dataset_root(dataset_root_dir, subject, sequence, frame):
    return os.path.join(dataset_root_dir, subject, sequence, 'images', frame)

def get_img_fname(img_dir, subject, sequence, frame, view, image_file_ext):
    return os.path.join(img_dir, '%s.%s.%s.%s' % (sequence, frame, view, image_file_ext))

def get_calibration_dir_from_calib_dir(calibration_dir, subject, sequence):
    return os.path.join(calibration_dir, subject, sequence)

def get_calibration_dir_from_dataset_root(dataset_root_dir, subject, sequence):
    return os.path.join(dataset_root_dir, subject, sequence, 'meshes', '*')

def get_scan_fname_from_scan_dir(scan_dir, subject, sequence, frame):
    return os.path.join(scan_dir, subject, sequence, '%s.%s.npy' % (sequence, frame))

def get_scan_fname_from_dataset_root(dataset_root_dir, subject, sequence, frame):
    return os.path.join(dataset_root_dir, subject, sequence, 'meshes', '%s.%s.obj' % (sequence, frame))

def get_registration_fname(registration_root_dir, subject, sequence, frame):
    return os.path.join(registration_root_dir, subject, sequence, '%s.%s.ply' % (sequence, frame))

def get_global_mesh_fname(global_registration_root_dir, subject, sequence, frame):
    return os.path.join(global_registration_root_dir, subject, sequence, '%s.%s.ply' % (sequence, frame))
...
class FaceAlignDatasetMPI(data.Dataset):
    def __init__(self, 
        ...
        self.root_dir: str = dataset_root_dir
        self.load_stereo_images: bool = load_stereo_images
        self.load_color_images: bool = load_color_images
        ...
        if os.path.exists(image_dir):
            ...
            self.img_dir_func = get_img_dir_from_image_dir
        elif os.path.exists(dataset_root_dir):
            self.image_dir = dataset_root_dir
            self.img_dir_func = get_img_dir_from_dataset_root
        else:
            raise RuntimeError('Invalid image directory')

        if os.path.exists(calibration_dir):
            self.calibration_dir_base = calibration_dir
            self.calibration_dir_func = get_calibration_dir_from_calib_dir
        # elif os.path.exists(dataset_root_dir):
        #     self.calibration_dir_base = dataset_root_dir
        #     self.calibration_dir_func = get_calibration_dir_from_dataset_root
        else:
            raise RuntimeError('Invalid calibration directory', calibration_dir)

        if os.path.exists(scan_dir):
            self.scan_dir_base = scan_dir
            self.scan_fname_func = get_scan_fname_from_scan_dir
        # elif os.path.exists(dataset_root_dir):
        #     self.scan_dir_base = dataset_root_dir
        #     self.scan_fname_func = get_scan_fname_from_dataset_root
        else:
            self.scan_fname_func = None
    ...
    def img_dir(self, subject, sequence, frame):
        return self.img_dir_func(self.image_dir, subject, sequence, frame)

    def img_fname(self, subject, sequence, frame, view):
        img_dir = self.img_dir(subject, sequence, frame)
        return get_img_fname(img_dir, subject, sequence, frame, view, self.image_file_ext)

    def calibration_dir(self, subject, sequence):
        return self.calibration_dir_func(self.calibration_dir_base, subject, sequence)

    def scan_fname(self, subject, sequence, frame):
        if self.scan_fname_func is None:
            return ''
        return self.scan_fname_func(self.scan_dir_base, subject, sequence, frame)

    def registration_fname(self, subject, sequence, frame):
        if not self.registration_root_dir:
            return ''
        return get_registration_fname(self.registration_root_dir, subject, sequence, frame)

    def global_mesh_fname(self, subject, sequence, frame):
        if not self.global_registration_root_dir:
            return ''
        return get_global_mesh_fname(self.global_registration_root_dir, subject, sequence, frame)
    ...
    def read(self, index):
        ...
        scan_fname_path = self.scan_fname(subject, sequence, frame)
        if scan_fname_path and self.scan_vertex_count > 0:
            v_sampled = self.load_scan_vertices(scan_fname_path)
            data['v_scan'] = torch.from_numpy(np.array(v_sampled).astype(np.float32))

        # Load registration
        registration_fname_path = self.registration_fname(subject, sequence, frame)
        if registration_fname_path:
            # print(f'Load registration from {registration_fname_path}')
            data['registration_fname'] = registration_fname_path
            try:
                registration = Mesh(filename=registration_fname_path)
                if not to_meters:
                    registration.v[:] *= 1000 # FLAME registrations are in meters, if to_meters is false, convert them to milimeters
            except:
                print(f'Unable to load registration {registration_fname_path}')
        ...
        global_mesh_fname_path = self.global_mesh_fname(subject, sequence, frame)
        if global_mesh_fname_path:
            # print(f'Load global mesh from {global_mesh_fname_path}')
            try:
                global_mesh = Mesh(filename=global_mesh_fname_path)
                if not to_meters:
                    global_mesh.v[:] *= 1000 # FLAME registrations are in meters, if to_meters is false, convert them to milimeters
            except:
                print(f'Unable to load global mesh {global_mesh_fname_path}')
...
class FaMosDataset(data.Dataset):
    def __init__(self, 
                 data_root: Path, 
                 split: str, 
                 ...
                 load_rgb: bool = False,
                 load_flame: bool = False,
                 load_scan: bool = False,
                 **kwargs):

        super().__init__()
        assert split in ['train', 'val', 'test'], f"Invalid split {split}, must be one of ['train', 'val', 'test']"
        
        split_root, data_list_fname = self._get_split_meta(data_root, split)
        kwargs.update({
            'data_list_fname': data_list_fname,
            'dataset_root_dir': Path(data_list_fname).parent,
        })
        
        if load_calib:
            kwargs['calibration_dir'] = split_root / "calibrations"
        
        if load_depth:
            kwargs['depth_dir'] = split_root / "depthfm"    
            
        if load_rgb:
            kwargs['image_dir'] = split_root / "downsampled_images_4"
            
        if load_flame:
            kwargs['registration_root_dir'] = split_root / "registrations"
            
        if load_scan:
            kwargs['scan_dir'] = split_root / "sampled_scan_points"
          
        print(f" Arguments for FaceAlignDatasetMPI: {kwargs}")
            
        self.raw_data: FaceAlignDatasetMPI = FaceAlignDatasetMPI(**kwargs)
        
    def _get_split_meta(self, famos_root: Path, split: str):
        split_root: Path = famos_root / "training_data"
        if split == 'test':
            split_root = famos_root / "test_data"
            
        # parse `data_list_fname`
        if split == 'test':
            jsons = list(split_root.glob("*.json"))
            assert len(jsons) == 1, f"Expected exactly one json file in {split_root}, found {len(jsons)}"
            data_list_fname = str(jsons[0])
        else:
            # 2 jsons for val and train
            json_kw = "val" if split == "val" else "train"
            jsons = list(split_root.glob(f"*_{json_kw}.json"))
            assert len(jsons) == 1, f"Expected exactly one json file matching '*_{json_kw}_*.json' in {split_root}, found {len(jsons)}"
            data_list_fname = str(jsons[0])
    ...
    # def persist(self):
    #     if self.use_cache:
    #         torch.save(self.cache, self.cache_dir)
    
    def _read_img_with_camera(self, subject, sequence, frame, calib_fname, to_meters=False):
        image_fname = self.raw_data.img_fname(subject, sequence, frame, get_filename(calib_fname))
        if not os.path.exists(image_fname):
            print('Image file not found - %s' % image_fname)
            return None
        try:
        ...
        camera = load_mpi_camera(calib_fname, self.raw_data.image_resize_factor, to_meters=to_meters)
        matting_path = self._matte_dir(subject, sequence, frame, get_filename(calib_fname))
        matting_image = Image.open(matting_path).convert("L") if "_C" in get_filename(calib_fname) else None
        
        if camera is None:
            return None

        if (image.shape[0] != camera['image_size'][0]) or (image.shape[1] != camera['image_size'][1]):
            image = cv2.resize(image, (camera['image_size'][1], camera['image_size'][0]), interpolation=cv2.INTER_AREA)

        if camera['image_size'][0] > camera['image_size'][1]:
            # The dataset contains images of landscape and portrait images of resolutions (A x B) and (B x A). 
            # To unify the images for batch handling, rotate all portrait images to landscape.
            image, camera = rotate_image(image, camera)
            matting_image = matting_image.rotate(90, expand=True) if matting_image is not None else None

        image = torch.FloatTensor(torch.from_numpy(image.astype(np.float32))).permute(2,0,1).contiguous() # (3,H,W) range (0,1) only rgb
        matting = torch.FloatTensor(torch.from_numpy(np.array(matting_image).astype(np.float32))).unsqueeze(0) / 255. if matting_image is not None else None # (1,H,W) range (0,1)
        intrinsics = torch.FloatTensor(torch.from_numpy(camera['intrinsics'].astype(np.float32)))
        extrinsics = torch.FloatTensor(torch.from_numpy(camera['extrinsics'].astype(np.float32)))
        radial_distortion = torch.FloatTensor(torch.from_numpy(camera['radial_distortion'].astype(np.float32)))
        camera_center = torch.FloatTensor(torch.from_numpy(camera['camera_center'].astype(np.float32)))
    ...
    def _depth_dir(self, subject, sequence, frame):
        return self.raw_data.root_dir / "depthfm" / subject / sequence / f"frame_{frame}.npy"
    
    def _matte_dir(self, subject, sequence, frame, view: str):
        return self.raw_data.root_dir / "matting" / subject / sequence / frame / f"{sequence}.{frame}.{view}.png"
        
    def _load_depth(self, index):
        subject, sequence, frame = self.raw_data.split_list[index]
        depth_path = self._depth_dir(subject, sequence, frame)
    ...
    def _load_image(self, index):
        ...
        # Read calibration files
        calib_fnames = sorted(glob.glob(os.path.join(self.raw_data.calibration_dir(subject, sequence), '*.tka')))
        ...
        for calib_fname in calib_fnames:
            if (not self.raw_data.load_stereo_images) and ('_A' in get_filename(calib_fname) or '_B' in get_filename(calib_fname)):
                continue
            if (not self.raw_data.load_color_images) and ('_C' in get_filename(calib_fname)):
                continue
            if get_filename(calib_fname) in self.raw_data.calibration_blacklist:
                continue

            img_with_camera = self._read_img_with_camera(subject, sequence, frame, calib_fname, to_meters=to_meters)
            
            if img_with_camera is not None:
                if '_C' in get_filename(calib_fname):
                    color_images.append(img_with_camera['image'])
                    matting_images.append(img_with_camera['matting'])
                    color_camera_intrinsics.append(img_with_camera['intrinsics'])
                    color_camera_extrinsics.append(img_with_camera['extrinsics'])
                    color_camera_distortions.append(img_with_camera['radial_distortion'])
                    color_camera_centers.append(img_with_camera['camera_center'])
        ...
        if len(color_images) > 0:
            color_images = torch.stack(color_images, dim=0)
            matting_images = torch.stack(matting_images, dim=0)
            color_camera_intrinsics = torch.stack(color_camera_intrinsics, dim=0)
            color_camera_extrinsics = torch.stack(color_camera_extrinsics, dim=0)
            color_camera_distortions = torch.stack(color_camera_distortions, dim=0)
            color_camera_centers = torch.stack(color_camera_centers, dim=0)
            ...
            color_camera_intrinsics = torch.tensor([])
            color_camera_extrinsics = torch.tensor([])
            color_camera_distortions = torch.tensor([])
            color_camera_centers = torch.tensor([])
            
        depth_images = self._load_depth(index)
        depth_images = torch.stack([depth_images], dim=1) if depth_images is not None else torch.tensor([])
        ...
        if (self.raw_data.scan_fname != '' and self.raw_data.scan_fname_func):
            # Load scan
            scan_fname = self.raw_data.scan_fname(subject, sequence, frame)
            v_sampled = self.raw_data.load_scan_vertices(scan_fname)
            result['v_scan'] = torch.from_numpy(np.array(v_sampled).astype(np.float32))

        if (self.raw_data.registration_fname != ''):
            # Load registration
            registration_fname = self.raw_data.registration_fname(subject, sequence, frame)
        
            if os.path.exists(registration_fname):
                result['registration_fname'] = registration_fname
                try:
                ...
                v_registration, f_registration = registration.v, registration.f
                result['v_registration'] = torch.from_numpy(v_registration.astype(np.float32))
                result['f_registration'] = torch.from_numpy(f_registration.astype(np.int64))

                if self.raw_data.mesh_sampler is not None:
                    for level in range(1,self.raw_data.mesh_sampler.get_number_levels()):
                        v_registration, f_registration = self.raw_data.mesh_sampler.downsample(v_registration, return_faces=True)

                result['v_reg_sampled'] = torch.from_numpy(v_registration.astype(np.float32))
                result['f_reg_sampled'] = torch.from_numpy(f_registration.astype(np.int64))
